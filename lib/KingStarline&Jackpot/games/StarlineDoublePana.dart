...
class _StarlineSingleDigitBetScreenState
  ...
  Future<void> _placeFinalBids() async {
    // Check if the widget is mounted before the API call to prevent a state update on a disposed widget
    if (!mounted) return;
    ...
    try {
      ...
      // Check mounted after the asynchronous call completes
      if (!mounted) return;
      ...
      // Check mounted after the catch block
      if (!mounted) return;
      _showMessage(
      ...
      // Ensure this final state update is also guarded
      if (mounted) {
        setState(() {
          _isApiCalling = false;
        });
