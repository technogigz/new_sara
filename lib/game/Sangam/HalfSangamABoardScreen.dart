...
class _HalfSangamABoardScreenState extends State<HalfSangamABoardScreen> {
  ...
  void _showConfirmationDialog() {
    ...
    // Corrected to show Panna-Digit format in the dialog
    List<Map<String, String>> bidsForDialog = _bids.map((bid) {
      return {
        "digit": "${bid['closePanna']!}", // Panna format
        "pana":
            "", // Panna field is not needed here as it's combined with digit
        "points": bid['points']!,
        "type": '--',
        "sangam": bid['sangam']!,
        "jodi": "",
  ...
  Future<Map<String, dynamic>> _placeFinalBids() async {
    ...
    try {
      ...
      if (result['status'] == true) {
        final int newBalance =
            (result['data']?['wallet_balance'] as num?)?.toInt() ??
            (_walletBalance - currentBatchTotalPoints);
        await _bidService.updateWalletBalance(newBalance);
      }
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final String displaySangam =
                                '${bid['closePanna']!} - ${bid['openDigit']!}'; // Panna - Digit format
                            return Container(
